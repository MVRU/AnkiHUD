...
# Definimos el nombre raíz del addon ("AnkiHUD") para que la configuración
...
class AnkiHUDSettings(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Cargar configuración usando ADDON_NAME
...
def inject_custom_styles(web_content, context):
    ...
    # Leemos config usando ADDON_NAME
...
def my_render_deck_node(self, node, ctx):
    ...
    # Chequeo Global (usando ADDON_NAME)
    config = mw.addonManager.getConfig(ADDON_NAME) or {}
    if not config.get("global_progress_enabled", True):
        return html_original

    # Chequeo Individual (Preferencias del mazo)
    deck = mw.col.decks.get(deck_id)
